...
def main(project_name):
    ...
    for i in tqdm(range(num_index)):
        ...
        t.add_item(i, (sum(embedder.get_vector(target_file[:-4], pos=p) for p in range(3)) / 3.0).tolist())

    dir_index = '_embed_index'
    os.makedirs(dir_index, exist_ok=True)

    with open(os.path.join(dir_index, 'test_names_{}.json'.format(project_name)), 'w') as f:
        json.dump(index_names, f)

    t.build(100)
    t.save(os.path.join(dir_index, 'test_features_{}.ann'.format(project_name)))
