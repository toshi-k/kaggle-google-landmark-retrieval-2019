...
def main(project_name, aux_projext_name):
    ...
    logger = Logger('_02_embed_test_{}'.format(project_name))
    logger.info('=' * 50)

    dir_prj = os.path.join('..', project_name[:-7])
    dir_aux = os.path.join('..', aux_projext_name[:-7])

    with open(os.path.join(dir_prj, '_embed_index', 'test_names_{}.json'.format(project_name)), 'r') as f:
        prj_index_names = json.load(f)

    with open(os.path.join(dir_aux, '_embed_index', 'test_names_{}.json'.format(aux_projext_name)), 'r') as f:
        aux_index_names = json.load(f)

    prj_u = AnnoyIndex(512, metric='euclidean')
    prj_u.load(os.path.join(dir_prj, '_embed_index', 'test_features_{}.ann'.format(project_name)))

    aux_u = AnnoyIndex(512, metric='euclidean')
    aux_u.load(os.path.join(dir_aux, '_embed_index', 'test_features_{}.ann'.format(aux_projext_name)))
    ...
    with open(os.path.join(dir_index, 'test_names_{}.json'.format(new_prj_name)), 'w') as f:
        json.dump(index_names, f)

    t.build(100)
    t.save(os.path.join(dir_index, 'test_features_{}.ann'.format(new_prj_name)))
