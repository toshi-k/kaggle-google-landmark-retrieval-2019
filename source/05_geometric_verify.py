...
def sort_by_delf(target_row):
    ...
    # num_fail_index = np.sum(array_inliers == -1)
...
def main():
    ...
    target = '../submission/submission_img_model_delf_model.csv'

    pre_submission_chunk = pd.read_csv(target, chunksize=chunk_size)
    # nrows=64

    list_images = list()
    list_ids = list()

    num_chunk = -(-len(pd.read_csv(target)) // chunk_size)
    ...
    for pre_submission in tqdm(pre_submission_chunk, position=-1, desc='ALL', total=num_chunk):
        ...
        # sort_by_delf(list_pre_submission[1])
        # exit()
    ...
    output_path = '../submission/submission_img_model_delf_model_verified.csv'
